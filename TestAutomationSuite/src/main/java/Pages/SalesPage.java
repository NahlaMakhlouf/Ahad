...
public class SalesPage extends PageBase {
	...
	private By addNewSalesReturnPageTitle =By.xpath("//div[@class='d-flex justify-content-center px-10 py-3 fs-1 ']");
	...
	private By returnWithReference = By.id("hasReference");
	private By returnWithoutReference = By.id("hasNoReference");
	private By cashBack = By.id("kt_create_invoice_form_invoice_type_cashback");
	private By proceedBtn = By.xpath("//button[@type='submit']");
	private By referenceInvoiceNumberInput = By.id("react-select-2-input");
	private By listLoader = By.id("loading_indicator");
	...
	private By returnedQuantityField = By.id("Returned quantity 0");
	...
	private By submitReturnInvoiceBtn = By.xpath("//button[@type=\"submit\"]");
	private By createSuccessMessage = By.xpath("//div[text()=\"تم إنشاء الفاتورة بنجاح!\"]");
	private By importSuccessMessage = By.xpath("//div[text()=\"تم رفع الملف بنجاح. سيتم عرض البيانات بعد إنتهاء العملية\"]");
	...
	public void addNewDraftInvoice(String invoiceDate, String paymentDate, String client, String product,
		...
		driverWait.until(ExpectedConditions.elementToBeClickable(invoiceDateField)).sendKeys(invoiceDate);
		driver.findElement(invoiceDateField).sendKeys(Keys.ENTER);

		driverWait.until(ExpectedConditions.elementToBeClickable(paymentDateField)).sendKeys(paymentDate);
	...
	public void selectReturnWithReferenceInvoice(String referenceInvoiceNumber) {
		driverWait.until(ExpectedConditions.visibilityOfElementLocated(returnWithReference)).click();
		driverWait.until(ExpectedConditions.elementToBeClickable(proceedBtn)).click();
		driverWait.until(ExpectedConditions.visibilityOfElementLocated(referenceInvoiceNumberInput)).sendKeys(referenceInvoiceNumber);
		driverWait.until(ExpectedConditions.visibilityOfElementLocated(listLoader));
		driverWait.until(ExpectedConditions.invisibilityOfElementLocated(listLoader));
		driverWait.until(ExpectedConditions.visibilityOfElementLocated(referenceInvoiceNumberInput)).sendKeys(Keys.ENTER);
		driverWait.until(ExpectedConditions.elementToBeClickable(proceedBtn)).click();

	}
	
	public String getAddNewSalesReturnPageTitle() {
		return driverWait.until(ExpectedConditions.visibilityOfElementLocated(addNewSalesReturnPageTitle)).getText();
	}

	public void addNewDraftReturnWithReferenceInvoice(String invoiceDate, String paymentDate, int returnedQuantity) {
		driverWait.until(ExpectedConditions.elementToBeClickable(invoiceDateField)).sendKeys(invoiceDate);
		driver.findElement(invoiceDateField).sendKeys(Keys.ENTER);
		...
		driverWait.until(ExpectedConditions.elementToBeClickable(returnedQuantityField)).sendKeys(Keys.BACK_SPACE);
		driver.findElement(returnedQuantityField).sendKeys(String.valueOf(returnedQuantity));
		
		driverWait.until(ExpectedConditions.elementToBeClickable(submitReturnInvoiceBtn)).click();

	}
	
	public void addNewIssuedReturnWithReferenceInvoice(String invoiceNumber, String invoiceDate, String paymentDate, int returnedQuantity) {
		driverWait.until(ExpectedConditions.visibilityOfElementLocated(invoiceStatusToggle)).click();
	...
	public boolean isCreateInvoiceSuccessMessageDisplayed() {
		driverWait.until(ExpectedConditions.visibilityOfElementLocated(createSuccessMessage));
		return true;
	}

	public void selectReturnWithNoReferenceInvoice() throws InterruptedException {
		driverWait.until(ExpectedConditions.elementToBeClickable(returnWithoutReference)).click();
		Thread.sleep(6000);
		driverWait.until(ExpectedConditions.elementToBeClickable(proceedBtn)).click();
	...
	public void importSalesInvoices(String path) {
		...
		driverWait.until(ExpectedConditions.visibilityOfElementLocated(importSuccessMessage));
		driverWait.until(ExpectedConditions.invisibilityOfElementLocated(importSuccessMessage));
